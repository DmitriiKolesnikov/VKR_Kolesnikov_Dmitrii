...
df = pd.read_csv('data_full.csv', encoding='utf-8')
...
features_center = ['Инфляция', 'Ключевая ставка в квадрате',
                    'Доля просроченных кредитов (NPL%)', 'Рентабельность активов (ROA%)', 'Размер банка (лог активов)', 'Доля банка в активах системы']
...
X_test_logit = sm.add_constant(X_test[['Инфляция', 'Ключевая ставка в квадрате',
                    'Доля просроченных кредитов (NPL%)', 'Рентабельность активов (ROA%)', 'Размер банка (лог активов)', 'Доля банка в активах системы']])
...
# Feature importances для Random Forest
...
prec_log = precision_score(y_test, y_pred_logit)
rec_log = recall_score(y_test, y_pred_logit)
